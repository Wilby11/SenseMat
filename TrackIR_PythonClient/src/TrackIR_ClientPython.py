...
class cHeadTrackingGUI:
    ...
    def __init__(self, master):
        ...
        self.recenterButton = tk.Button(
            self.frame, text="Recenter (Zero)", command=self.RecenterTracking)
        self.recenterButton.grid(row=4, column=1, columnspan=2, sticky='ew', pady=5)
    ...
    def StartTimer(self):
        # update ever 17ms roughly ~60fps (changed to 8ms for ~120fps)
        self.timerId = self.master.after(8, self.UpdateTrackingInfo)
    ...
    # Recenter the tracking data to establish a new zero-point origin
    def RecenterTracking(self):
        result = npc.ReCenter()
        
        if (result == NPRESULT.NP_OK):
            self.AddLine("NPClient : Tracking origin recentered to 0.")
        else:
            self.AddLine("NPClient : Error recentering tracking data.")
        ...
        if result == NPRESULT.NP_OK :
            ...
            if tid.Status == npc.NPSTATUS_REMOTEACTIVE:
                ...
                if (self.frameSig != tid.FrameSignature):
                    ...
                    # Get the precise hardware stopwatch time
                    raw_perf_time = time.perf_counter()
                    
                    # If this is the first frame, anchor the absolute Unix time
                    ...
                    # Add the precise elapsed time to the absolute Unix start time
                    current_unix_time = self.unix_start_time + elapsed_seconds
                    ...
                    self.AddTopLine(("X: " + str(round(x,1)) + "cm "
                                    + "Y: " + str(round(y,1)) + "cm "
                                    + "Z: " + str(round(z,1)) + "cm | "
                    ...
                    self.recorded_data.append([
                        ...
                        tid.FrameSignature,
                        tid.Status,
                        current_unix_time, x, y, z, pitch, yaw, roll])
